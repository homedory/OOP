...
public class Cafe {
	...
	public void rentOutGame(String name) {
		...
		this.money += gamesInCafe[gameIdx].getRentingCost();  // money 대여비용만큼 증가
	...
	public void buyGame(Game game) {
		if(game.getPrice() > money) { //돈이 부족한 경우 구입 불가
	...
	public void printCafeDetails() {
		System.out.println("Money: " + this.money);  //잔고 출력
		...
		for (int i = 0; i < rentedOutGames.length; i++) { //빌려간 게임들 출력
			System.out.print(rentedOutGames[i]);
		}
	}
	...
	public void repairGame(String name) {
		...
		money -= gamesInCafe[gameIdx].getRepairCost(); //수리비용만큼 차감
